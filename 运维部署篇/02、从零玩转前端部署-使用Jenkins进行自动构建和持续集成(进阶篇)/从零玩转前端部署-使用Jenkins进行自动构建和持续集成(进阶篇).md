本系列上一篇主要讲了利用 Nginx 进行简单的静态资源部署，这种方式需要在代码更新以后手动的把代码重新上传到服务器进行部署，方式比较原始。在现代的敏捷式的软件开发和管理中，产品需要快速的进行更新和迭代，传统的迭代和交付方式已无法满足现实的需要。因此，一种新的交付和部署方式就产生了，简称为 CI、CD。

# 1、CI/CD 和 Jenkins 的基本认识
...
“Jenkins”则是在进行 CI/CD 的工具之一，也是在进行项目持续集成和部署最常用的工具。

# 2、创建待部署的项目并推送至远程仓库

## 2.1、使用 Vue-Cli 创建一个待部署的 Vue 项目
...
此时，在浏览器 url 输入 localhost:8080 就可以访问到刚才创建的初始化项目了。

## 2.2、创建一个远程仓库并将项目代码推送至远程仓库

- 在码云/github/gitlab 上创建一个项目
...
将获取到的密码填入浏览器`jenkins`初始页面的密码输入框内，点击继续即可进入到`jenkins`插件配置界面。为了更加清楚的知道`Jenkins`常用的插件有哪些，以及每个插件的作用，我们这里这里选择第二个选项`选择插件来安装`。
<img src="./img/jenkins05.png" width="100%">

### 创建初始管理员用户

选择`选择插件来安装`后，会进入到一个选择安装插件的界面，这里我们也选择`无`，具体需要的插件我们后续可以通过管理页面去安装，选择`继续`后，就出现了下面创建第一个用户的界面：
...
至此，`Jenkins`平台的搭建就完成了！具体功能则交给相关的插件。
...
在安装界面可以勾选上安装完成后自动重启。
<img src="./img/jenkins12.png" width="100%">

也可以安装完成之后，在浏览器地址栏端口号后加上`/restart`重启`Jenkins`即可，也可以通过如下命令行重启：
```bash
service jenkins restart
```

之后，重新登陆`Jenkins`界面就会变成全中文界面。
<img src="./img/jenkins13.png" width="100%">
